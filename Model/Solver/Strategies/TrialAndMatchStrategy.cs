...
{
    public string Name => "Trial and match";
    
    public StrategyLevel Difficulty => StrategyLevel.ByTrial;
        ...
        for (int row = 0; row < 9; row++)
        {
            for (int col = 0; col < 9; col++)
            {
                if (strategyManager.Sudoku[row, col] == 0 &&
                    strategyManager.PossibilitiesAt(row, col).Count <= _maxNumberOfPossibility)
                {
                    if(ApplyChanges(strategyManager, RunSimulation(strategyManager, row, col,
                        strategyManager.PossibilitiesAt(row, col)))) return;
                }
            }
        }
    }
    ...
    private int[,] RunSimulation(IStrategyManager strategyManager, int row, int col, IEnumerable<int> possibilities)
    {
        ...
        {
            Model.Solver.Solver simulation = strategyManager.Copy();
