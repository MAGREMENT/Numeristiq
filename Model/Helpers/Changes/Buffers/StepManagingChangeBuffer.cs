...
public class StepManagingChangeBuffer<TVerifier, THighlighter> : IChangeBuffer<TVerifier, THighlighter> where TVerifier : IUpdatableSolvingState where THighlighter : ISolvingStateHighlighter
{
    ...
    public bool IsManagingSteps => true;
    ...
    public void PushCommit(BuiltChangeCommit<THighlighter> commit)
    {
        ...
        _producer.StepHistory.AddFromReport(commit.Report, commit.Changes, commit.Maker, state);
    }
}
...
{
    void Push(ICommitMaker pusher, List<ChangeCommit<TVerifier, THighlighter>> commits, IStepManagingChangeProducer<TVerifier, THighlighter> producer);
}
...
public class FirstOnlyPushHandler<TVerifier, THighlighter> : IPushHandler<TVerifier, THighlighter> where TVerifier : IUpdatableSolvingState where THighlighter : ISolvingStateHighlighter
{
    public void Push(ICommitMaker pusher, List<ChangeCommit<TVerifier, THighlighter>> commits, IStepManagingChangeProducer<TVerifier, THighlighter> producer)
    {
        ...
        producer.StepHistory.AddFromReport(commit.Builder.BuildReport(commit.Changes, state), commit.Changes, pusher, state);
    }
}
...
public class UnorderedAllPushHandler<TVerifier, THighlighter> : IPushHandler<TVerifier, THighlighter> where TVerifier : IUpdatableSolvingState where THighlighter : ISolvingStateHighlighter
{
    public void Push(ICommitMaker pusher, List<ChangeCommit<TVerifier, THighlighter>> commits, IStepManagingChangeProducer<TVerifier, THighlighter> producer)
    {
        ...
        foreach (var commit in commits)
        {
            ...
            producer.StepHistory.AddFromReport(commit.Builder.BuildReport(impactfulChanges, state), impactfulChanges, pusher, state);
        }
    }
}
...
{ 
    public void Push(ICommitMaker pusher, List<ChangeCommit<TVerifier, THighlighter>> commits, IStepManagingChangeProducer<TVerifier, THighlighter> producer)
    {
        ...
        producer.StepHistory.AddFromReport(best.Builder.BuildReport(best.Changes, state), best.Changes, pusher, state);
    }
}
