...
{
    public BridgingStrategy() : base("Bridging", Difficulty.Easy, InstanceHandling.UnorderedAll)
    {
    }
        ...
        for (int row = 0; row < data.Nonogram.RowCount; row++)
        {
            ...
            for (int c = space.Start; c <= space.End; c++)
            {
                ...
                if (last >= 0)
                {
                    ...
                    {
                        data.ChangeBuffer.Commit(new BridgingReportBuilder(
                            Orientation.Horizontal, row, c, last, space.FirstValueIndex));
        ...
        for (int col = 0; col < data.Nonogram.ColumnCount; col++)
        {
            ...
            for (int r = space.Start; r <= space.End; r++)
            {
                ...
                if (last >= 0)
                {
                    ...
                    {
                        data.ChangeBuffer.Commit(new BridgingReportBuilder(
                            Orientation.Vertical, col, r, last, space.FirstValueIndex));
                        if(StopOnFirstCommit) return;
                    }
                }
